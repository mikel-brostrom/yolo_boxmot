...
class MultiScaleFPN(pl.LightningModule):
    def __init__(self, num_classes=80, learning_rate=1e-3):
        ...
        # Extract layers to get feature maps at different scales
        ...
        # FPN layers
        # 1x1 conv layers to reduce channel dimensions
        ...
        self.head_small = nn.Conv2d(
            in_channels=256,
            out_channels=self.num_anchors_per_scale * (5 + num_classes),
            kernel_size=1
        )
        self.head_medium = nn.Conv2d(
            in_channels=256,
            out_channels=self.num_anchors_per_scale * (5 + num_classes),
            kernel_size=1
        )
        self.head_large = nn.Conv2d(
            in_channels=256,
            out_channels=self.num_anchors_per_scale * (5 + num_classes),
            kernel_size=1
        )
    ...
    def forward(self, x, decode=False):
        ...
        predictions = {}
        feature_maps = {'small': p3, 'medium': p4, 'large': p5}
        ...
        for scale in ['small', 'medium', 'large']:
            ...
            pred = pred.view(
                batch_size,
                H * W * self.num_anchors_per_scale,
                5 + self.num_classes
            )
    ...
    def decode_predictions(self, predictions):
        ...
        for scale in ['small', 'medium', 'large']:
            ...
            for b in range(batch_size):
                ...
                decoded_scores.append(torch.tensor(scores, device=device))
                decoded_labels.append(torch.tensor(labels, device=device))
    ...
    def training_step(self, batch, batch_idx):
        ...
        for b in range(batch_size):
            ...
            for t in range(valid_target_cls.size(0)):
                ...
                # Compute IoU with all anchors across all scales
                ious = []
                anchor_indices = []
                scales = []
                for scale in ['small', 'medium', 'large']:
        ...
        for scale in ['small', 'medium', 'large']:
            ...
            if obj_mask.any():
                ...
                class_loss = F.cross_entropy(
                    pred_class_scores_flat[obj_mask.view(-1)],
                    target_class_flat[obj_mask.view(-1)],
                    ignore_index=-1,
                    reduction='mean'
                )
            else:
